...
comando = input("Que quiere hacer?\n")
split = comando.split()
temp = split.pop(0)
new = ""
...
if temp == "put":
    ...
    if row:
        ...
        if column[0].replace("'", "") in JSON:
            ...
            a[column[1].replace("'", "")] = [new[3].replace("'", ""), time.time()]
            x = JSON
        else:
            a = {}
            a[column[1].replace("'", "")] = [new[3].replace("'", ""), time.time()]
        ...
        JSON = {}
        column = new[2].split(":")
        JSON[column[0].replace("'", "")] = {column[1].replace("'", ""): [new[3], time.time()]}
    ...
    for x in json_object:
        for y in x:
            ...
            for z in rows:
                ...
                for w in columns:
                    values = columns[w]
                    print(y+ "              column="+z+":"+w+ ", "+ "timestamp="+values[-1]+", " + "value="+values[0])
